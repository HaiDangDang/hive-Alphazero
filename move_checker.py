...
def is_valid_move(state, old_tile, new_tile, check_move_break_hive=True):
    import pieces
    ...
    if new_tile is not None:
        check_Bettle = (new_tile.has_pieces()
                        and type(state.moving_piece)
                        is pieces.Beetle)

    if check_Bettle and type(old_tile.pieces[-1]) == pieces.Beetle and \
            old_tile.axial_coords == (99, 99):
        return False
    ...
    if state.turn == 1:
        ...
        # print(queen_is_on_board(state, old_tile))
...
def move_is_not_blocked_or_jump(state, old_tile, new_tile):  # check for each pathfinding move
    ...
    if old_tile in new_tile.adjacent_tiles:
        dist = 1
    if dist == 1 and len(overlap_tiles) == 0:  # restrict jumps
        if old_tile.has_pieces():
            if type(old_tile.pieces[-1]) in [pieces.Spider]:
                return True
            elif type(old_tile.pieces[-1]) in [pieces.Beetle]:
                if len(new_adjacents_with_pieces) - 1 != 0:
                    return True
...
def is_straight_line(old_coords, new_coords):
    ...
    return q1 == q2 or r1 == r2 or delta_y == delta_x
...
def player_has_no_moves(state):
    ...
    for old_tile in player_piece_tiles:
        for new_tile in open_adjacent_tiles:
            # print(is_valid_move(state, old_tile, new_tile))
